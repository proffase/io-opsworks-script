...
if args.parameter == 'empty':
    ...
    # print(result)
    result = subprocess.run('cd /home/ubuntu/mountpoint/io-opsworks-script/.git/hooks && touch post-merge && chmod 755 post-merge', stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8')
    result = subprocess.run('cd /home/ubuntu/mountpoint/io-opsworks-script/.git/hooks && echo "#!/bin/sh\nexec sudo systemctl restart apache2" | tee post-merge', stdout=subprocess.PIPE, shell=True).stdout.decode('utf-8')
else:
